...
backup-utils Pod는 SSH로 Github Enterprise Server에 접근 가능한 구조이기 때문에, `kubectl exec` 이후 GHES 관리 셸로 들어가는 점프 박스(Jump Box)로 활용할 수 있습니다.

### 점프 박스로 접속

```bash
# backup-utils Pod에 접속
kubectl exec -it -n backup-utils <POD_NAME> -- bash

# Pod 내부에서 GHES 관리 셸로 SSH 접속
ssh -p 122 -l admin -i /ghe-ssh/id_ed25519 <GHE_SERVER_IP>
```

### 접속 예시

backup-utils Pod에서 GHES Primary 서버로 SSH 접속한 결과입니다. 일반적으로 SSH는 TCP 22 포트를 사용하지만, GHES의 관리용 SSH 포트는 TCP 122를 사용합니다. 자세한 내용은 [Network ports](https://docs.github.com/en/enterprise-server@latest/admin/configuring-settings/configuring-network-settings/network-ports) 공식문서를 참고하세요.

```bash
root@github-backup-utils-12345678-abcde:/ghe-ssh# ssh -p 122 -l admin -i /ghe-ssh/id_ed25519 10.0.0.100
...
documented operations procedures only.

admin@github-primary:~$
```

점프 박스로 접속하면 GHES 관리 셸에서 다양한 운영 작업을 수행할 수 있습니다.

이 방식은 별도의 Bastion 호스트 없이도 Kubernetes 클러스터 내에서 GHES에 안전하게 접근할 수 있어 운영 효율성을 높여줍니다.
