...
클러스터 내부의 로그 수집기로 잘 사용하고 있던 Promtail이 공식적으로 지원종료된 소식을 [Loki 공식문서](https://grafana.com/docs/loki/latest/send-data/promtail/#promtail-agent)와 [[promtail] is promtail chart abandoned? #3614](https://github.com/grafana/helm-charts/issues/3614) 이슈를 통해 접하게 되었습니다.
...
DaemonSet은 모니터링 에이전트(Grafana Alloy, Prometheus Node Exporter, Datadog Agent), 로그 수집 에이전트(Fluentd, Fluent Bit), 네트워크 플러그인(Calico, Cilium), 보안 에이전트(Falco) 등 클러스터의 모든 노드에서 실행되어야 하는 시스템 컴포넌트에 주로 사용됩니다.

Kubernetes는 특정 노드에 taint를 설정하여 일반 Pod의 스케줄링을 제한합니다. 마스터/컨트롤 플레인 노드뿐만 아니라 GPU 노드, 고성능 노드 등 특수 목적의 노드에도 taint가 설정되어 있는 경우가 많습니다. 예를 들어 Grafana Alloy와 같은 모니터링 에이전트는 GPU 노드를 포함한 모든 노드의 메트릭과 로그를 수집해야 하므로, DaemonSet이 이러한 노드들에도 배포되려면 toleration 설정이 필요합니다.

모든 노드에 alloy DaemonSet을 배포하는 가장 간단한 방법은 다음과 같이 모든 taint를 tolerate하도록 설정하는 것입니다:
...
이 설정은 key와 effect를 지정하지 않아 모든 taint를 tolerate하며, DaemonSet이 클러스터의 모든 노드에 배포됩니다.
