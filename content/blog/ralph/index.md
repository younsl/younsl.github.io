...
PROMPT.md is a declarative requirements document. You describe *what* you want, not *how* to do it step by step. It should contain:
...
- Do NOT use third-party routers. Use net/http.
...
This prompt is specific. Every endpoint has its request/response format, status codes, and edge cases defined. The project layout and rules are clear. There is nothing left for the agent to guess.
...
A key lesson from Ralph users: declarative requirements work better than step-by-step instructions. Tell the agent what the end result should look like. Don't tell it which files to open or which commands to run. Bad requirements lead to bad output.
