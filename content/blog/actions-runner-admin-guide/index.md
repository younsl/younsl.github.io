...
### Actions Runner Controller Deprecation

**이 문서는 Deprecated된 Actions Runner Controller(ARC)를 다룹니다.**

현재 이 문서에서 다루는 `actions-runner-controller`와 `RunnerDeployment`는 **더 이상 사용되지 않습니다(Deprecated)**. GitHub는 [공식 발표](https://github.com/actions/actions-runner-controller/discussions/2775)를 통해 새로운 아키텍처로 마이그레이션할 것을 권장하고 있습니다.

새로운 프로젝트에서는 `gha-runner-scale-set-controller`(컨트롤러)와 `gha-runner-scale-set`(러너 스케일 셋) 조합을 사용하세요. 새로운 아키텍처는 더 나은 성능, 확장성, 그리고 지속적인 지원을 제공합니다.

다만 `gha-runner-scale-set`은 러너당 라벨을 하나만 설정할 수 있다는 치명적인 제약이 있습니다. 기존 ARC의 `RunnerDeployment`에서는 `["t3a.large", "ubuntu-20.04", "v2.306.0"]`과 같이 여러 라벨을 한 러너에 자유롭게 붙일 수 있었지만, `gha-runner-scale-set`에서는 불가능합니다. 이 때문에 여러 라벨을 조합해서 러너를 선택하던 기존 워크플로우를 수정하거나, 라벨별로 별도의 러너 스케일 셋을 추가로 구성해야 할 수 있습니다.
...
![Helm chart architectures](./0.png)
...
Actions Runner가 정상 동작하는지 테스트하기 위해 샘플 워크플로우를 작성합니다.

모든 GitHub Actions 워크플로우 파일은 레포지터리의 .github/workflows/ 디렉터리에 위치해야 합니다. 아래는 GitHub 공식문서의 [Quickstart Workflow](https://docs.github.com/en/actions/quickstart#creating-your-first-workflow)를 기반으로 한 테스트용 워크플로우입니다.
...
제가 Grafana에 등록해서 사용하고 있는 대시보드 [19382](https://grafana.com/grafana/dashboards/19382-horizontalrunnerautoscalers/) 화면입니다.
...
- [Autoscaling Runner Scale Sets mode is now generally available #2775](https://github.com/actions/actions-runner-controller/discussions/2775): 새로운 Runner Scale Sets 모드의 GA 발표 및 구 버전 지원 종료 안내
- [Actions Runner Controller Github](https://github.com/actions/actions-runner-controller/tree/master#documentation): Actions Runner Controller는 공식문서가 친절한 편이라 위 링크만 봐도 설치, 구성에는 전혀 문제가 없었습니다.
