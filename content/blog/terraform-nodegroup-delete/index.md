...
AWS 프로필을 설정하고 Terraform 상태 파일을 백업합니다:
...
Terraform state에 등록된 모든 리소스 목록을 확인합니다 (EKS module v17 기준으로 한 노드그룹은 ASG, Launch Template, Instance Profile 3개의 리소스로 구성됩니다):
...
`reindex-workers.sh` 스크립트를 EKS 테라폼 모듈이 위치한 디렉토리에 같이 위치시킵니다.
...
스크립트 상단에서 삭제할 노드그룹 인덱스와 전체 리소스 개수를 설정합니다:
...
print_deletion_info() {
  ...
  for resource_base in "${resource_bases[@]}"; do
...
delete_resource() {
  ...
  for resource_base in "${resource_bases[@]}"; do
    echo "Deleting $resource_base[$deletion_target_index]"
...
reindex_resources() {
  echo ""
  echo "=== Reindexing resources ==="
  for resource_base in "${resource_bases[@]}"; do
    echo "Processing $resource_base..."
    for (( i=deletion_target_index + 1; i<total_resources; i++ )); do
      old_index=$i  # 현재 인덱스
      new_index=$((i - 1))  # 새로운 인덱스
      echo "  Moving $resource_base[$old_index] -> $resource_base[$new_index]"
...
스크립트를 실행하여 삭제할 리소스와 재배열할 리소스 목록을 확인합니다:

```bash
sh reindex-workers.sh
```

스크립트가 삭제할 리소스와 재배열할 리소스 목록을 출력하며 사용자 확인을 요청합니다:
...
스크립트가 대상 리소스를 State에서 제거하고 나머지 리소스의 인덱스를 재배열합니다:
...
State 파일에서 리소스 인덱스가 올바르게 재배열되었는지 확인합니다:
...
State 재배열 후 실제 인프라와의 차이가 없는지 확인합니다:
...
인덱스가 정상적으로 재배열되었다면 State 파일과 실제 인프라가 일치하여 아래와 같이 변경 사항 없음을 확인할 수 있습니다:
...
직접 Management Console 또는 AWS CLI를 사용해서, `terraform state rm`으로 제거했던 ASG와 Launch Template 리소스를 삭제합니다. `terraform state rm`은 State 파일에서만 리소스를 제거하며 실제 AWS 리소스는 삭제하지 않습니다. Terraform이 더 이상 해당 리소스를 인식하지 못하므로, AWS Console이나 CLI로 수동 삭제해야 비용 발생과 리소스 누수를 방지할 수 있습니다.
...
- [can't apply if trying to remove worker_groups_launch_template #887](https://github.com/terraform-aws-modules/terraform-aws-eks/issues/887): Github issue
